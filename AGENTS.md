...
- **Scroll-driven motion is art-directed per composition, never a general `.reveal`.** One treatment repeated down consecutive sections is the templated version and flattens what it runs over; a page earns motion by giving each element role its own range. Transform-only — opacity leaves the text readable to axe at zero contrast for as long as it runs — behind `@supports (animation-timeline: view())` and a `prefers-reduced-motion: no-preference` query, so a browser without scroll timelines renders the page in place. Reset the animation on the element that carries it, or the reset guards nothing.
