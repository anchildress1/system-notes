...
- **axe has no font-size rule, and contrast passes just as happily at 11px.** The notes index shipped its own evidence at 11.5px and `/projects` listed an exhibit's stack at 11px, both through a full green suite. Prose — sentence-case running text — reads at or above 16px on every route; `--fine` (11px) is for tracked uppercase captions a reader glances at, never for a sentence. The floor is asserted against computed styles at 280/390/1440 in `integration.spec.ts`, over **(route, state) pairs, not routes** — a route loaded only in its initial state leaves every state reached by interaction unmeasured, and that is where a restored brief at 11px and an offline notice at 14px survived. Add the pair when you add the state. Gate each on its own prose being visible, or an empty result means the nodes were absent rather than passing. Classify prose by ROLE, never by length: a six-word cutoff exempted whole sentences for being short. Three tokens of two letters or more is a clause; a tracked uppercase string and anything inside `a, button, summary` is a label. A state gated at BUILD time — the credential-less intake notice — cannot be reached from the harness at all; raise those by inspection and say so. A number copied out of the stylesheet into a unit test drifts with it.
...
- **A selected note never reaches the URL,** and never opens in an overlay or a modal. It opens in place, inside the row that names it — never lifted out of the list into a reader above it, and never scrolled to. **The selection is the page** — the queue derives its page from the open note and never stores one, so the two cannot disagree and the rail's board brings the queue with it. A page with nothing open on it is the defect; holding a chosen page beside the selection is what produces one. Paging therefore moves the reader, and reveals without reporting an insights click, because pressing Next is navigation and not a result the reader picked.
- **The open row's control cannot collapse it, so it says so.** Carry `aria-disabled` and make the handler a no-op there. A bare `aria-expanded="true"` on a control that ignores Enter is a broken promise, and re-selecting re-sent the insights click for a note already open.
- **Name a row's control from its title alone**, via `aria-labelledby`. Named from its contents, every heading in the outline reads ordinal, project and date before any title, and heading navigation becomes a list of identical prefixes. Playwright's `name` is a substring match by default and passes straight through this — assert `exact: true`.
- **A stretched mask needs its aspect, and a bound.** `--stroke-brush-y` is 16x56 under `preserveAspectRatio='none'`; stretched past about 1:8 the wobble flattens to a pixel, which is the 1px rule nothing decorative may be. Cap the height — anchoring `top` and `bottom` to a box whose height the content decides lets a wrapped title stretch it right back.
- **Every mask carries its `-webkit-` prefix.** Unprefixed alone drops the mark on Safari before 16.4 and the fallback is a solid bar. Only the WebKit project runs `cross-browser.spec.ts`, so nothing else catches it.
...
- **Scroll-driven motion is art-directed per composition, never a general `.reveal`.** One treatment repeated down consecutive sections is the templated version and flattens what it runs over; a page earns motion by giving each element role its own range. Transform-only — opacity leaves the text readable to axe at zero contrast for as long as it runs — behind `@supports (animation-timeline: view())` and a `prefers-reduced-motion: no-preference` query. Reset the animation on the element that carries it, or the reset guards nothing. **A browser with no scroll timelines gets the same motion, not none of it** — a `requestAnimationFrame` fallback scrubs the identical transform off scroll position, gated behind the same reduced-motion query and torn down the moment `@supports` starts matching. Progressive enhancement here means an equivalent experience everywhere, not a static page for whichever engine lacks the native feature.
...
- Lighthouse gates two profiles: desktop at 98 over `/`, `/projects`, `/about`, and mobile at 89 over those three plus `/notes`. Mobile layout and target sizes are still measured in pixels in the mobile spec; a score is not a substitute for either.
- Mobile floors sit at 92 locally because the ceiling is 0.96. LCP is bound by ~187 KB of framework JS under a 4× CPU multiplier, and the two routes whose LCP element is an image run ~300ms behind the two whose LCP element is text. Preload, eager loading and `fetchPriority` were each measured on those images: every one traded load delay for render delay and left LCP unmoved.
- Gate dropped to 89 (2026-09-24): GHA measured `/projects` at 0.91/0.91/0.90/0.90/0.91, below the local 92 floor — CI-hardware gap, not a regression. Restore to 92 once re-measured against CI directly.
