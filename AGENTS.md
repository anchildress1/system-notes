...
- **axe has no font-size rule, and contrast passes just as happily at 11px.** The notes index shipped its own evidence at 11.5px and `/projects` listed an exhibit's stack at 11px, both through a full green suite. Prose — sentence-case running text — reads at or above 16px on every route; `--fine` (11px) is for tracked uppercase captions a reader glances at, never for a sentence. The floor is asserted against computed styles at 280/390/1440 in `integration.spec.ts`, over the four routes in their SUCCESS state only — a warning, an offline notice or an error page renders prose the sweep never reaches, so raise those by inspection. Gate the sweep on the prose being present, or an empty result means the nodes were absent rather than passing. A number copied out of the stylesheet into a unit test drifts with it.
...
- **A stretched mask needs its aspect.** `--stroke-brush-y` is 16x56 under `preserveAspectRatio='none'`; span it down a whole note and the wobble flattens to a pixel, which is the 1px rule nothing decorative may be. Keep it near 1:8.
