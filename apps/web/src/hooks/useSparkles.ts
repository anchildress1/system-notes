    ...
    const handleInteraction = async (clientX: number, clientY: number) => {
      ...
      try {
        ...
        for (let i = 0; i < count; i++) {
          ...
          particle.scale.set(Math.random() * baseScale + 0.1); // NOSONAR - visual randomness

          particle.x = x + (Math.random() - 0.5) * 20; // NOSONAR - visual randomness (jitter)
          particle.y = y + (Math.random() - 0.5) * 20; // NOSONAR - visual randomness (jitter)
          particle.alpha = 1;

          const angle = Math.random() * Math.PI * 2; // NOSONAR - visual randomness
          const velocity = Math.random() * 5 + 2; // NOSONAR - visual randomness
          ...
          particle.decay = Math.random() * (isMobile ? 0.05 : 0.03) + 0.01; // NOSONAR - visual randomness
